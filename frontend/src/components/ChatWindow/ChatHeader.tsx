...
const ChatHeader: React.FC = () => {
  ...
  const name = useMemo(() => {
    ...
    } else if ('channel' in res) {
      return res.channel.name;
    } else if ('group' in res) {
    ...
    <div className="py-2 px-3 bg-gray-100 flex justify-between items-center border-b">
      <div className="flex items-center">
        {'user' in data.data ? (
          ...
          <ChatAvatar chat={data.data.group} />
        )}
        <div className="ml-4">
      ...
      <Space size="large">
        <Tooltip title="Start video call">
          <Button shape="circle" icon={<VideoCameraOutlined />} onClick={handleVideoCall} />
        </Tooltip>
