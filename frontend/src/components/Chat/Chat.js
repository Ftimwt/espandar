...
const Chat = () => {
  ...
  useEffect(() => {
    const fetchAESKey = async () => {
      try {
        ...
        setError('خطا در دریافت کلید رمزنگاری');
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        const endpoint = type === 'user' ? `/messages/user/${id}` : type === 'group' ? `/messages/group/${id}` : `/messages/channel/${id}`;
    ...
    const fetchTagSuggestions = async () => {
      try {
        const [usersRes, filesRes, workflowsRes] = await Promise.all([
          ...
          const workflows = workflowsRes.data.map((wf) => ({
            id: `workflow_${wf.ID}`,
            display: wf.Title,
            type: 'workflow',
          }));
      ...
      // اتصال WebSocket
      socketRef.current = new WebSocket(`ws://localhost:8080/ws?Authorization=${encodeURIComponent(token)}&receiver_id=${id}`);
      ...
      socketRef.current.onmessage = (event) => {
        ...
        switch (message.event) {
          case 'connect_success':
            ...
            break;
          case 'new_message':
            setMessages((prev) => [...prev, { ...message.data, content: decryptMessage(message.data.Content) }]);
            break;
          case 'webrtc_offer':
    ...
    const handleSendMessage = async () => {
      ...
      if (newMessage.trim()) {
        const encryptedMessage = CryptoJS.AES.encrypt(newMessage, AESKey).toString();
    ...
    const handleOffer = async (offer, from) => {
      try {
        ...
        localVideoRef.current.srcObject = stream;
        stream.getTracks().forEach((track) => pc.addTrack(track, stream));
        pc.ontrack = (event) => {setRemoteStreams((prev) => ({ ...prev, [from]: event.streams[0] }));
      };
      pc.onicecandidate = (event) => {
        if (event.candidate) {
          socketRef.current.send(JSON.stringify({
  ...
  const startMultiUserCall = async (conferenceId) => {
    try {
      ...
      conferenceMembers.forEach((member) => {
        const pc = new RTCPeerConnection({
          iceServers: [
            { urls: 'stun:stun.l.google.com:19302' },{ urls: 'turn:your-turn-server', username: 'user', credential: 'pass' },
          ],
        });
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
        ...
        {messages.map((msg) => (
          <Paper
            ...
            sx={{
              ...
              mb: 2,
              maxWidth: '70%',
              alignSelf: msg.sender_id === parseInt(userId) ? 'flex-end' : 'flex-start',
            }}
          >
            <Typography>
      ...
      <Box sx={{ p: 2, borderTop: '1px solid #ccc' }}>
        ...
        >
          <Mention trigger="@" data={tagSuggestions} />
          <Mention trigger="#" data={tagSuggestions} />
        </MentionsInput>
        <Box sx={{ display: 'flex', gap: 1, mt: 1 }}>
          ...
          {type === 'user' && (
            <>
              <Button variant="contained" onClick={startVideoCall} disable={inCall}>
                شروع تماس
              </Button>
              {inCall && (
                <>
