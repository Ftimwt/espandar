...
const Chat = () => {
    const { type, selectedModel } = useHome();
    ...
    const connectWebSocket = () => {
        ...
        const wsUrl = `ws://localhost:8080/ws?receiver_id=${id}&Authorization=${encodeURIComponent(
            token
        )}`;
            ...
            try {
                ...
                switch (eventName) {
                    case "connect_success":
                        console.log(
                            "Chat: Connected to WebSocket:",
                            messageData
                        );
                        break;
                    case "new_message":
                        console.log(
                            "Chat: Processing new_message, data:",
                            messageData
                        );
                        if (
                            messageData &&
                            ...
                            console.warn(
                                "Chat: Invalid new_message received:",
                                message
                            );
                        }
                        break;
        ...
        socketRef.current.onclose = (event) => {
            console.log(
                "Chat: WebSocket disconnected, code:",
                event.code,
                "reason:",
                event.reason
            );
            ...
            if (event.code === 1006) {
                console.log(
                    "Chat: Connection refused, checking server status..."
                );
            }
        ...
        const fetchMessages = async () => {
            try {
                ...
                if (!Array.isArray(response.data)) {
                    console.warn(
                        "Chat: fetchMessages response is not an array:",
                        response.data
                    );
                    setMessages([]);
                    return;
            ...
            try {
                ...
                const files = filesRes.data
                    ...
                    .map((file) => ({
                        id: `file_${file.ID}`,
                        display:
                            file.FilePath.split("/").pop() || "unnamed_file",
                        type: "file",
                    }));
    ...
    const handleSendMessage = async () => {
        ...
        for (const match of fileMatches) {
            ...
            if (file)
                tags.push({
                    type: "file",
                    id: parseInt(file.id.replace("file_", "")),
                    name: match[1],
                });
        }
        ...
        for (const match of workflowMatches) {
            ...
            if (workflow)
                tags.push({
                    type: "workflow",
                    id: parseInt(workflow.id.replace("workflow_", "")),
                    name: match[1],
                });
        }
            ...
            selectedFiles.length > 0
                ? selectedFiles[0].type.startsWith("image")
                    ...
                    : selectedFiles[0].type.startsWith("audio") ||
                      selectedFiles[0].type === "audio/webm"
                    ? "voice"
                    : "video"
                : "text"
        );
        ...
        try {
            console.log("Chat: Sending message, id:", id);
    ...
    const startRecording = async () => {
        try {
            ...
            mediaRecorderRef.current.onstop = () => {
                ...
                setSelectedFiles([
                    new File([audioBlob], "voice_message.webm", {
                        type: "audio/webm",
                    }),
                ]);
        ...
        if (
            ...
            navigator.mediaDevices
                ...
                .catch((err) => {
                    ...
                    setError(
                        `خطا در دسترسی به دوربین/میکروفون: ${err.message}`
                    );
                    setOpenSnackbar(true);
                });
        }
    ...
    const startVideoCall = async () => {
        try {
            ...
            pc.ontrack = (event) => {
                console.log(
                    "Received remote stream for user:",
                    id,
                    event.streams[0]
                );
        ...
        <Box sx={{ display: "flex", flexDirection: "column" }}>
                        ...
                        >
                            ...
                            {Array.isArray(msg.Files) && msg.Files.length > 0
                                ? msg.Files.map((file) => {
                                      ...
                                      const isVideo =
                                          file.Type === "video" && !isVoice;
                                      return (
                                          <Box
                                              key={
                                                  file.ID ||
                                                  `${msg.ID}-file-${file.FilePath}`
                                              }
                                          >
            ...
            >
                ...
                <DialogActions>
                    ...
                    {conferenceLink && (
                        <Button
                            onClick={() => startMultiUserCall("conference_id")}
                        >
                            شروع کنفرانس
                        </Button>
                    )}
                    <Button onClick={() => setOpenConferenceDialog(false)}>
                        بستن
                    </Button>
                </DialogActions>
            </Dialog>
