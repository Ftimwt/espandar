...
func SetupRoutes(
	...
	r.GET("/ws", websocket.SocketHandler)
	...
	protected.Use(jwt.JWTAuthMiddleware(db))
	{
		...
		protected.POST("/profile/image", authCtrl.UploadProfileImage)
		...
		protected.GET("/channel/:channel_id/members", channelCtrl.GetChannelMembers)
		...
		protected.GET("/channel/info/:id", channelCtrl.GetChannel)
		...
		protected.GET("/group/:group_id/members", groupCtrl.GetGroupMembers)
		...
		protected.GET("/group/info/:id", groupCtrl.GetGroup)
		protected.DELETE("/group/:group_id", groupCtrl.DeleteGroup)
		protected.POST("/conference", messageCtrl.CreateConference)
