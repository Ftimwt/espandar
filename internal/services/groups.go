...
func NewGroup(repo *repositories.Group, repoChannel *repositories.Channel, repoUser *repositories.User) *Group {
	return &Group{
		repo:        repo,
...
func (g Group) MarkAsRead(userID uint, channelID uint, messageID uint) (int64, error) {
	// TODO: check user access
	return g.repoChannel.MarkAsRead(userID, messageID)
}
